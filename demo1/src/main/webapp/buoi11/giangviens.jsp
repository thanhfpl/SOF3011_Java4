<%-- Created by IntelliJ IDEA. User: hangnt Date: 27/3/24 Time: 18:19 To change
this template use File | Settings | File Templates. --%> <%@ taglib prefix = "c"
      ...
      <tbody>
        <c:forEach items="${a}" var="gv">
          <tr>
            ...
            <td>${gv.queQuan}</td>
            <td>
              <a href="/giang-vien/remove?idXoa=${gv.id2343}">
                <button class="btn btn-primary">Xóa</button></a
              >
              <a href="/giang-vien/detail?idDetail=${gv.id2343}"
                ><button class="btn btn-warning">Detail</button></a
              >
              <a href="/giang-vien/view-update?idUpdate=${gv.id2343}"
                ><button class="btn btn-danger">Update</button></a
              >
            </td>
          </tr>
        </c:forEach>
